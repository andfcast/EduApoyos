...
{
    public class RolControllerIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
    {
        private readonly WebApplicationFactory<Program> _factory;
        private readonly System.Net.Http.HttpClient _client;

        public RolControllerIntegrationTests(WebApplicationFactory<Program> factory)
        {
            _factory = factory;
        ...
        public async Task Get_DeberiaRetornarOkYLista_DeCuandoSeConsultaConTokenValido()
        {
            ...
            using (var scope = _factory.Services.CreateScope())
            {
                ...
                var existing = dbContext.Usuarios.FirstOrDefault(u => u.Email == "integration.rol@edu.co");
                if (existing == null)
                {
                    var usuarioPrueba = new Usuario(Guid.NewGuid(), "Integration Rol", "integration.rol@edu.co", BCrypt.Net.BCrypt.HashPassword("Password123*"), 2, DateTime.UtcNow);
            ...
            var response = await _client.GetAsync("/api/Rol");
            ...
            roles!.Select(r => r.Nombre).Should().Contain(new[] { "Asesor", "Estudiante" });
        }
    }
}